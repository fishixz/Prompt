...
async function execute(interaction) {
  ...
  if (member.roles.cache.has(role.id)) {
    return interaction.editReply(`⚠️ ${member} já possui o cargo ${role}.`);
  }

  await member.roles.add(role, `RochaSystem • ${interaction.user.tag}: ${reason}`);
  ...
  return interaction.editReply(`✅ ${role} foi adicionado a ${member}.`);
}

module.exports = { id: 'admin.cargo-adicionar', execute };
